    ...
    <ion-content :force-overscroll="false" class="bg-primary">
      ...
      <main class="pt-4 pb-8">
        ...
        <section v-else class="px-4">
          <div
            class="w-full h-[280px] rounded-3xl bg-black/10 flex items-center justify-center text-tertiary/70"
          >
        ...
        >
          <button
            ...
            :class="
              index === inlineIndex ? 'w-6 bg-tertiary' : 'w-2.5 bg-tertiary/40'
            "
      ...
      <transition name="lightbox-fade">
        <div v-if="isGalleryOpen" class="fixed inset-0 z-[1200] bg-black">
          <div
            class="absolute top-0 left-0 right-0 z-10 flex items-center justify-between p-4 text-white"
          >
            <button
              class="custom-menu-button bg-white/10"
              @click="closeGallery"
            ...
            <span class="text-sm font-medium"
              >{{ galleryIndex + 1 }} / {{ venueImages.length }}</span
            >
            <button
              class="custom-menu-button bg-white/10"
              @click="closeGallery"
...
const onPointerDown = (event: PointerEvent, index: number) => {
  ...
  pointerStart.set(event.pointerId, {
    x: event.clientX,
    y: event.clientY,
    moved: false,
  })
...
const onPointerMove = (event: PointerEvent, index: number) => {
  ...
  if (pointers.size >= 2 && pinchState.value) {
    ...
    const nextScale = Math.min(
      4,
      Math.max(1, pinchState.value.startScale * ratio),
    )
...
const openGallery = async (index: number) => {
  ...
  if (galleryRef.value) {
    galleryRef.value.scrollTo({
      left: galleryRef.value.clientWidth * index,
      behavior: 'auto',
    })
  }
}
